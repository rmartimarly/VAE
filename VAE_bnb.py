...
def main():
    ...
    """
    wandb.init(project="vae_test")
    
    # track hyperparameters and run metadata
    wandb.config = { "learning_rate": 1e-3,"architecture": "CNN", "batch_size": 128,
        "dataset": "mammo", "num_epochs": 10 }
    ...
    if (wandb.config["dataset"] == "mnist"):
        ...
        test_loader = torch.utils.data.DataLoader(
            ...
            batch_size=1)
    if (wandb.config["dataset"] == "mammo"):
        
        wandb.config["num_epochs"] = 50
        # using only normal images  
        #path = "/mnt/mia_images/breast/iceberg_selection2/HOLOGIC/normal_roi"
        path = "~/data/omi-db_selection/normal_ROI_hologic/normal_roi"
        print ("Mammo - Batch Size ",  wandb.config["batch_size"])
        # transform=transforms.ToTensor()
...
# break
    ...
    for epoch in range(wandb.config["num_epochs"]):
        for idx, data in enumerate(train_loader, 0):
            ...
            loss.backward()
            optimizer.step()
            # Log the original and reconstructed images to wandb
            wandb.log({"Original Image": [wandb.Image(imgs[0], caption="Original Image")],
           "Reconstructed Image": [wandb.Image(out[0], caption="Reconstructed Image")]})


        print('Epoch {}: Loss {}'.format(epoch, loss))
        # log metrics to wandb
        wandb.log({"loss": loss}) # could add FID similarity or other. 

    # Save network
    #torch.save(net, "testVAE_mammo_normal_"+str(wandb.config["num_epochs"])+".pth")
    wandb.unwatch()
    wandb.finish()
    torch.save(net, "testVAE_mnist_normal.pth")
